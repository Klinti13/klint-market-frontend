...
export default function MobileNav({ cartCount, user, onOpenAuth, onLogout }: MobileNavProps) {
  ...
  const [showConfirm, setShowConfirm] = useState(false); // STATE PER MODALIN
  ...
  return (
    <>
            ...
            <>
              <Link to="/profile" className={getLinkStyle('/profile')}>
                <div className={`w-10 h-10 flex items-center justify-center rounded-2xl transition-all ${location.pathname === '/profile' ? 'bg-emerald-500/10' : ''}`}>
                   <div className="w-6 h-6 rounded-full bg-emerald-500 flex items-center justify-center text-[10px] font-black text-slate-900 border border-emerald-400/30 leading-none uppercase">
                    {user.name[0]}
                  </div>
                </div>
      ...
      {/* MODAL I KONFIRMIMIT (I njëjti stil si në Desktop) */}
      {showConfirm && (
